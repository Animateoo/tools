...
const MediaVaultProject = (function () {
    ...
    const AUDIO_SUBFOLDER = "Audios";

    const LEGACY_AUDIO_SUBFOLDER = "Audio";
    ...
    function footageRelative(projectRoot, filePath) {

        try {

            return path.relative(projectRoot, path.resolve(filePath)).replace(/\\/g, "/").toLowerCase();
    ...
    function isInFootageSubfolder(projectRoot, filePath, folderName) {

        const rel = footageRelative(projectRoot, filePath);

        const prefix = "(footage)/" + folderName.toLowerCase() + "/";
    ...
    function getFootageSubfolder(fileName) {
        ...
        if (AUDIO_EXT.indexOf(ext) >= 0) return AUDIO_SUBFOLDER;
    ...
    function existingFootageFile(projectRoot, subfolder, fileName) {

        const dirs =

            subfolder === AUDIO_SUBFOLDER

                ? [AUDIO_SUBFOLDER, LEGACY_AUDIO_SUBFOLDER]
        ...
        for (i = 0; i < dirs.length; i++) {
            ...
            candidate = path.join(projectRoot, "(Footage)", dir, fileName);
     ...
     * Copy source file into projectRoot/(Footage)/{Audios|Footage|Images|Other}/
    ...
    function copyToProjectFootage(srcPath, projectRoot) {
        ...
        const inAudios = isInFootageSubfolder(projectRoot, srcPath, AUDIO_SUBFOLDER);

        const inLegacyAudio = isInFootageSubfolder(projectRoot, srcPath, LEGACY_AUDIO_SUBFOLDER);
        ...
        if (isPathUnderRoot(srcPath, projectRoot)) {
            ...
            if (subfolder !== AUDIO_SUBFOLDER || inAudios || !inLegacyAudio) {
        ...
        const existingPath = existingFootageFile(projectRoot, subfolder, fileName);

        if (existingPath && isInFootageSubfolder(projectRoot, existingPath, AUDIO_SUBFOLDER)) {

            try {

                const stSrc = fs.statSync(srcPath);

                const stDest = fs.statSync(existingPath);
        ...
        try {
            ...
            return {
                ...
                message: inLegacyAudio ? "copied_from_legacy_audio" : "copied"
